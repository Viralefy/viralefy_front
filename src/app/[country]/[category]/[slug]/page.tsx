...
function planNarrative(lang: string, cat: string, catLabel: string, qty: number, country: string): string[] {
  ...
  const NARR: Record<string, (q: number, country: string, label: string) => string[]> = {
    en: (q, ctry, label) => [
      ...
      `If anyone drops within 30 days we top your count back up at no cost. Pay in USD, EUR, BRL or crypto — invoices are emailed automatically.`,
    ],
    pt: (q, ctry, label) => [
...
export default async function PlanPage({ params }: { params: Promise<Params> }) {
  ...
  const jsonld: object[] = [
    ...
    },
    {
      ...
      offers: {
        "@type": "Offer",
        price: plan.prices?.["BRL"] ?? (plan.price_cents / 100).toFixed(2),
        priceCurrency: "BRL",
